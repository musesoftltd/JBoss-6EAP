...
def auditServersCommsHubThread(environment, servername, propertiesDictionary, bApplyRequiredChanges) :
...
def auditServersCommsHub(environment, servers, propertiesDictionary, bApplyRequiredChanges) :
    # merge global properties into dict - deliberately overwriting local with global dict all values
    
    strThreadPoolId = "auditServersCommsHub"
    for servername in servers:
        # Create new threads
        scatterThread(strThreadPoolId, auditServersCommsHubThread, args=(environment, servername, propertiesDictionary, bApplyRequiredChanges))
