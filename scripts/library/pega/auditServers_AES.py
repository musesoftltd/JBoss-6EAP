...
def auditServersAESThread(environment, servername, propertiesDictionary, bApplyRequiredChanges) :
    ...
    if (AllDatasources) :        
        ...
        for ds in AllDatasources:
            auditObjectMolecule1.auditObjectAtoms.append(auditObjectAtom(servername, runtimeProperties["username"], runtimeProperties["password"], "JDBC URL - AES - " + ds, "/subsystem=datasources/data-source=" + ds + "/", "connection-url", runtimeProperties["targetDSUrlAes"], bApplyRequiredChanges))

def auditServersAES(environment, servers, propertiesDictionary, bApplyRequiredChanges) :
    # merge global properties into dict - deliberately overwriting local with global dict all values
    
    strThreadPoolId = "auditServersAES"
    for servername in servers:
        # Create new threads
        scatterThread(strThreadPoolId, targetMethod=auditServersAESThread, args=[environment, servername, propertiesDictionary, bApplyRequiredChanges]) 
