...
def auditServersBatchThread(environment, servername, propertiesDictionary, bApplyRequiredChanges) :
...
def auditServersBatch(environment, servers, propertiesDictionary, bApplyRequiredChanges) :
    # merge global properties into dict - deliberately overwriting local with global dict all values
    
    strThreadPoolId = "auditServersBatch"
    for servername in servers:
        # Create new threads
        scatterThread(strThreadPoolId, targetMethod=auditServersBatchThread, args=[environment, servername, propertiesDictionary, bApplyRequiredChanges]) 
