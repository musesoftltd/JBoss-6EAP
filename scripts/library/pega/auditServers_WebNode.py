...
def auditServersWebNodeThread(environment, servername, propertiesDictionary, bApplyRequiredChanges) :
...
def auditServersWebNode(environment, servers, propertiesDictionary, bApplyRequiredChanges) :
    # merge global properties into dict - deliberately overwriting local with global dict all values
    
    strThreadPoolId = "auditServersWebNode"    
    for servername in servers:
        # Create new threads
        scatterThread(strThreadPoolId, auditServersWebNodeThread, args=(environment, servername, propertiesDictionary, bApplyRequiredChanges))
